...
final class SearchAllEventsResponse extends Equatable {
  const SearchAllEventsResponse({
  ...
  factory SearchAllEventsResponse.fromJson(Map<String, dynamic> json) {
    if (json
        case {
          'content': Map<String, dynamic> content,
          'status': bool status,
        }) {
      return SearchAllEventsResponse(
        content: Content.fromJson(content),
        status: status,
      );
    } else {
      throw const FormatException(
        '''Failed to deserialize Search Events response.
